...
namespace Snake
{
    ...
    {
        m_displayPort.send(std::make_unique<EventT<DisplayInd>>(Food::sendPlaceNewFood(x,y)));
    }

     void Controller::sendClearOldFood()
     {
         m_displayPort.send(std::make_unique<EventT<DisplayInd>>(Food::sendClearOldFood());
     }
    ...
    namespace
    {
        ...
        {
            return (isVertical(direction) and Direction_DOWN == direction) or (isHorizontal(direction) and Direction_RIGHT == direction);
        }
